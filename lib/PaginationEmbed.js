 ...
 * @property {String} [deleteButton] Emoji which should be used as the delete button. Defaults to: 🗑
 * @property {String} [firstButton] Emoji which should be used as the first page button. Defaults to: ⏮
 * @property {String} [lastButton] Emoji which should be used as the last page button. Defaults to: ⏭
 * @property {String} [backButton] Emoji which should be used as the back button. Defaults to: ⬅
 * @property {String} [forthButton] Emoji which should be used as the forth button. Defaults to: ➡
 ...
 * @property {Boolean} [cleanUp] Whether to auto remove reactions in at end or not. Defaults to: false
 */
/**
...
class PaginationEmbed {
    ...
    run() {
        this.handler.on('reacted', async (event) => {
            let em;
            if(event.emoji.id === null) em = event.emoji.name;
            else if(event.emoji.animated === true) em = `a:${event.emoji.name}:${event.emoji.id}`;
            else em = `${event.emoji.name}:${event.emoji.id}`;
